...
describe("QUERY", () => {
  ...
  test("should query records filtered with metadata filter", async () => {
    const initialVector = [6.6, 7.7];
    const initialData = {
      id: 34, vector: [6.6, 7.7], metadata: {
        city: "Istanbul", population: 1546000, geography: {
          continent: "Asia",
        },
      }
    };
    ...
    //This is needed for vector index insertion to happen. When run with other tests in parallel this tends to fail without sleep. But, standalone it should work without an issue.
    await sleep(2000);
    const res = await new QueryCommand<{ city: string, population: number, geography: { continent: string } }>({
      vector: initialVector,
      topK: 1,
  ...
  test("should narrow down the query results with filter", async () => {
    ...
    const initialData = [{
      id: 1, vector: exampleVector,
      metadata: {
        animal: "elephant", tags: ["mammal"], diet: "herbivore"
      }
    }, {
      id: 2, vector: exampleVector,
      metadata: {
        animal: "tiger", tags: ["mammal"], diet: "carnivore"
      }
    }];
